        ...
        if(putDollarsForNextPeriods(
            timeTable, className, subject, day, room, nextPeriodName,startTime,endTime)):           
                timeTable.loc[(day,room), nextPeriodName] = str(
                    [className,'%s-%s'%(period[-1],subject),'Teacher','%s-%s'%(startTime,endTime)])
        else:
            return False
    return True
...
def allotLab(timeTable,className,subject,duration):
    ...
    for l in lab.readAllLabs().Name:
        for p in timeTable.columns:
            for d in timing.daysOfWeek():
                if((timeTable.loc[(d,l),p] == theDefaultEmptyChar())):
                    # Clash Resolve : one class in two rooms 
                    lectureList = list(timeTable.loc[d,p])
                    classesList = list()
                    for k in lectureList:
                        if(k != theDefaultEmptyChar()):
                    ...
                    if(subject[1] <= duration):
                        timeTable.loc[(d,l),p] = [
                            className,subject[0],'Teacher',('%s-%s'%(s,e))]
                        return True
                    else:
                        if(putDollarsForNextPeriods(
