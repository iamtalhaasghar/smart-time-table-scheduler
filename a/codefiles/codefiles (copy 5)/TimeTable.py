def createTimeTable():
    ...
    dataF.to_csv('%s/%s.csv'%(folder,'TimeTableFormat'))
...
def allotLab(timeTable,className,subject,duration):
    #TODO: Subject list pattern has been changed. remove this comment after correction
    import Lab
    import Timing
    import Teacher
    timeList = readTimeHeaders()
    ...
    for l in list(Lab.readAllLabs().Name):
        for p in timeTable.columns:
            for d in Timing.daysOfWeek():
                # TODO: check for desired lab{labName == l and}
