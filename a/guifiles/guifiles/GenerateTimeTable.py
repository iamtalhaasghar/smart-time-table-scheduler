...
class Ui_generateTimeTable(object):
            ...
            for p in programs:
                ...
                for c in classes:
                    self.timeTableInfo.append('Generating Time Table of : Depart: %s, Program: %s, Class: %s'
                                              % (d,p,c))
                    TimeTable.generateTimeTable(d, p, c)
                    self.timeTableInfo.append('Done!! for %s'% c)
        self.timeTableInfo.append('Time Table Generation was successful.')
    ...
    def eliminateDuplicates(self):
        ...
        import Filer
        import sqlite3
        p = '%s/web/%s.db' % (Filer.CURRENT_FOLDER, Filer.TIME_TABLE_FILE_NAME)
        
        import os
        if(os.path.exists(p)):
            c = sqlite3.connect(p)
            cr = c.cursor()
            cr.execute('Drop table TimeTable')
        c = sqlite3.connect(p)
        import TimeTable
        tTable = TimeTable.readTimeTable()
        tTable.to_sql('TimeTable',c)
        tTable.to_excel('%s/%s.xlsx' %(Filer.TIME_TABLE_FOLDER, Filer.TIME_TABLE_FILE_NAME))
        c.close()
